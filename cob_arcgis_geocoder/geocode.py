...
class CobArcGISGeocoder(object):
    ...
    def clean_df(self, df, address_field):
        ...
        clean_df = clean_df.where(clean_df == '-', None) 
    ...
    def reverse_geocode(self, address_df):
        ...
        if len(addresses_df_points.index) == 0: 
            ...
            matched_address_df = addresses_df[["address", "score", "attributes.Street_ID", "location.x", "location.y"]].sort_values(by="score", ascending=False).iloc[0]
    ...
    #             df.at[index, "flag"] = "No address provided in this field. Unable to geocode."
    #         else: 
    #             candidates = find_address_candidates(SingleLine=row[address_field])
    #             matched_address_df = pick_address_candidate(candidates)  
    ...
    #                 df.at[index, "SAM_ID"] = matched_address_df[["attributes.Street_ID"]][0]
